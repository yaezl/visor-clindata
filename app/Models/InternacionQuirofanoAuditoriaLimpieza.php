<?php

/**
 * Created by Reliese Model.
 */

namespace App\Models;

use Carbon\Carbon;
use Illuminate\Database\Eloquent\Model;

/**
 * Class InternacionQuirofanoAuditoriaLimpieza
 * 
 * @property int $id
 * @property int $created_by
 * @property int $modified_by
 * @property int $reserva_id
 * @property int|null $inicio_id
 * @property int|null $fin_id
 * @property Carbon $updated_at
 * @property Carbon $created_at
 * @property bool $borradoLogico
 * 
 * @property Usuario $usuario
 * @property InternacionQuirofanoAuditoriaTimer|null $internacion_quirofano_auditoria_timer
 * @property ReservaQuirofano $reserva_quirofano
 *
 * @package App\Models
 */
class InternacionQuirofanoAuditoriaLimpieza extends Model
{
	protected $table = 'internacion_quirofano_auditoria_limpieza';

	protected $casts = [
		'created_by' => 'int',
		'modified_by' => 'int',
		'reserva_id' => 'int',
		'inicio_id' => 'int',
		'fin_id' => 'int',
		'borradoLogico' => 'bool'
	];

	protected $fillable = [
		'created_by',
		'modified_by',
		'reserva_id',
		'inicio_id',
		'fin_id',
		'borradoLogico'
	];

	public function usuario()
	{
		return $this->belongsTo(Usuario::class, 'created_by');
	}

	public function internacion_quirofano_auditoria_timer()
	{
		return $this->belongsTo(InternacionQuirofanoAuditoriaTimer::class, 'fin_id');
	}

	public function reserva_quirofano()
	{
		return $this->belongsTo(ReservaQuirofano::class, 'reserva_id');
	}
}
